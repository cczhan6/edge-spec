...
def build_fixed_epoch_plan(
    ...
    *,
    ...
    normalized_batches = tuple(tuple(batch) for batch in batches if batch)
    return DipSDEpochPlan(
...
def optimize_epoch_plan(
    ...
    *,
    ...
    """Compatibility wrapper for the simulator until M17 wires paper inputs."""

    request_ids = tuple(sorted(int(request_id) for request_id in active_request_ids))
    if not request_ids:
        return DipSDEpochPlan((), {}, optimizer="paper_exact")
    users = tuple(
        DipSDUser(
            request_id=request_id,
            prefix_length=1,
            acceptance_estimate=float(acceptance_estimates.get(request_id, 0.5)),
            communication_latency_ms=0.0,
            draft_latency_scale=0.0,
            draft_latency_overhead_ms=1.0,
        )
    ...
    problem = DipSDProblem(
        ...
        min_draft_length=int(min_draft_length),
        max_draft_length=int(max_draft_length),
        initial_draft_length=int(min_draft_length),
        max_batch_size=int(max_batch_size),
        max_batch_count=int(max_batch_count),
    )
    return optimize_dip_sd(problem)
