...
动态 gamma 从 `speculation.gamma_candidates = [1, 2, 4, 6, 8]` 枚举，过滤超过剩余长度
的候选值，并最大化：
...
| `SpecEdge` | SpecEdge 机制级基线 | 线性 draft、server batch validation、proactive 线性 continuation、pipeline-aware scheduling，暂不使用 token tree |
| `server_only` | SpecEdge server-only 近似基线 | 请求级上传 prompt/下载最终输出；服务器侧 draft 后直接 target verify，不做 segment 级端边往返或 global batch |
...
- `sync_batch_sd`：使用 heterogeneous drafter、动态 gamma，并采用全局同步 batch verify。
  ...
  在同步 barrier 下的表现。
- `SpecEdge`：SpecEdge baseline 的机制级实现。当前实现不移植 SpecEdge token tree，而是使用线性 speculative draft、
  server batch validation、发送 validate 后立即启动的 proactive 线性 continuation，
  以及 pipeline-aware scheduling。SpecEdge 分支按
  `server verify time ~= edge draft time + RTT` 动态校准 draft depth；server 侧按
...
- `server_only`：对应 SpecEdge server-only baseline 的近似版本。每个请求按
  ...
  503.9 tok/s。当前仿真未移植原仓库 token tree，因此 `server_only` 采用固定线性
  draft depth，默认每轮 `gamma = specedge.max_beam_len = 4`，仅在剩余输出不足时截断。
...
- `avg_acceptance_rate`：真实接受 token / draft token；
...
- `speculation.gamma_candidates`：评估更保守或更激进的 lookahead；
...
- `SpecEdge` 表示 SpecEdge 线性 baseline，预期通过 server batch validation、
  proactive continuation 和 pipeline-aware draft-depth 校准减少 server idle bubble；
