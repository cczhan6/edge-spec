...
- 默认实验设置 `simulation.include_prefill: false`，请求在到达时即视为 decode-ready；
...
固定 verification forward 近似计费。默认主实验不计入首轮 prompt 上传、drafter
prefill 或 target prefill；非 target-only 方法的首个 segment 上传 payload 只包含
draft/tree payload token。若需要复现实验早期的 end-to-end prompt 口径，可显式设置
`simulation.include_prefill: true`。
...
if simulation.include_prefill:
  ...
  first draft/verify/target-only service includes the corresponding prefill term
```
...
默认 decode-only 口径下，非 target-only 方法的首个 segment 上行 `token_count` 只包含
segment payload token；后续 segment 同样只包含 draft/tree payload token。只有
`simulation.include_prefill: true` 时，首个 segment 上行才额外包含 `prompt_tokens`。
...
生成输出 token。请求完成时延按全部输出 token 生成完成并返回最终输出计算。语义由
target 真实 greedy decoding 决定，时延由 `target_only_ms` 和输出通信公式决定。
...
| `server_only` | SpecEdge-style server-only 近似基线 | 默认只下载最终输出；服务器侧树形 draft 后直接 target verify，不做 segment 级端边往返或 global batch；可显式切到线性近似 |
...
- `server_only` 默认不承担请求级 prompt 上传，只承担最终输出下载，不承担 segment 级端边往返；
