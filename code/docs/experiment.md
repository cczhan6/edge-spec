...
- `server_only_tree`：对应 SpecEdge-style server-only tree baseline。服务器侧使用固定
  ...
  proactive draft，最终输出仍由 target greedy 语义决定。
...
- `server_only_linear` 和 `server_only_tree` 消除了端边往返，但所有 draft/verify 都集中在服务器侧；
- `specedge_linear` 和 `specedge_tree` 通过端侧 drafter 分担 draft 计算，并引入端边通信、server batch validation 和 proactive continuation；
- `dip_sd` 通过 ordered batch pipeline 降低同步空泡，但 epoch barrier 和有序 batch 会影响尾部时延；
