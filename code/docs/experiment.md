...
语义和解析时延模型下的 steady-state autoregressive decoding 收益。

核心口径：

- 请求到达时即视为 decode-ready，端侧 drafter 和边缘 target 已分别建立该请求的 prefix KV cache；
- 不建模 prompt prefill computation、prompt transmission、TTFT、hidden state 或 prompt 通信；
- 请求完成时延、TPOT/TBT、goodput 和加速比均按 decode-only 口径统计；
...
固定 verification forward 近似计费。Prompt 编码和 prefix KV 建立只作为解码初始条件，
不进入虚拟时间或资源调度；非 target-only 方法的首个 segment 上传 payload 只包含
draft/tree payload token。
...
`target_only` 将请求视为已在边缘具备 decode 所需上下文，单个 target 服务资源自回归
生成输出 token。请求完成时延按全部输出 token 生成完成计算；decode-only 口径下不模拟
prompt 上传或最终输出下载。语义由 target 真实 greedy decoding 决定，时延由
`target_only_ms` 解析公式决定。
...
`speculation.unconfirmed_token_budget` 作为未确认 draft token 总预算 `B`。每次起草前，
调度器先计算当前 pending 链上仍处于 active 状态的 draft token 数，并把本轮动态
`gamma` 限制在剩余输出长度和剩余预算内。因此允许的在途 segment 数量不是固定常数，
而是由各段实际 `gamma` 和剩余预算自然决定。后续位置的 segment 到达边缘后，
...
`speculation.gamma_candidates = [1, 2, 4, 6, 8]` 枚举，过滤超过剩余长度和剩余
未确认 token 预算的候选值，并最大化：
...
对比基线使用 `docs/baseline_contract.md` 固定的 canonical 方法名：

| 方法 | 基线含义 | 设备池与行为 |
|---|---|---|
| `target_only` | 边缘自回归基线 | decode-ready 请求由单个 target 服务资源自回归生成 |
| `server_only_linear` | Server-only SD-Linear | 服务器侧线性 draft 后 target verify，draft/target 使用独立逻辑资源，无端边通信；主实验固定 `server_only.batch_size=1` |
| `specedge_linear` | SpecEdge-Linear | 端侧线性 draft、端边往返、server batch validation、proactive continuation |
| `dip_sd` | DiP-SD (Online Adaptation) | DiP-SD 论文优化器和同步 batch pipeline 在本项目在线请求调度框架下的适配 |
| `server_only_tree` | Server-only SD-Tree | 服务器侧 `specexec_approx` tree draft 后 target verify，无端边通信、无 proactive；主实验固定 `server_only.batch_size=1` |
| `specedge_tree` | SpecEdge-Tree | 端侧 `specexec_approx` tree draft、server batch validation、proactive continuation |
...
Legacy aliases `sync_batch_sd`、`SpecEdge` 和 `server_only` 仍可运行，用于历史结果和回归测试兼容；
主实验和验收使用上表 canonical 名称。

SD baseline 的具体解释：

- `server_only_linear`：对应集中式线性 speculative decoding。每个请求在服务器侧交替执行
  draft、target verify 和状态更新；draft 与 target 分别占用独立逻辑资源，但按官方
  server-only round loop 不引入 proactive 或跨轮 overlap。decode-only 口径下没有
  prompt 上传、segment 往返或最终输出下载。主实验固定 `server_only.batch_size=1`；
  官方 SpecEdge server-only 支持多请求 tree verification，但本项目当前尚未实现真实
  多请求 server-only verification batch，因此配置大于 1 会报错。
- `specedge_linear`：保留 SpecEdge 的端侧 drafter、端边通信、server batch validation、
...
- `dip_sd`：正式显示名为 `DiP-SD (Online Adaptation)`。它使用已实现的 DiP-SD
  论文优化器和同步批次流水，但运行在本项目的在线请求调度框架下。请求按照优化结果
  分组，每请求使用优化得到的 draft length；同一 batch 等待所有成员 ready，慢设备阻塞
  其所在 batch，不同 batch 之间允许 draft 与 verification 流水重叠。每个请求必须等当前
  verification 和 KV/state update 完成后才能开始下一轮；新请求按照当前 epoch/barrier
  在线接入规则进入后续优化周期。优化器只使用配置/历史估计，不读取未来 acceptance
  或未来 arrival。
- `server_only_tree`：对应 SpecEdge-style server-only tree baseline。服务器侧使用固定
  1.5B 级 `medium` drafter 交替执行 `specexec_approx` tree draft 和 target verify，
  使用独立 server draft GPU 与 server target GPU 逻辑资源。它不进入 global batch，
  不产生 proactive draft，默认 `server_only.batch_size=1`，最终输出仍由 target greedy
  语义决定。linear/tree 只改变候选结构，不改变资源和调度设置。
- `specedge_tree`：SpecExec-style 树形基线。主实验默认使用 `specexec_approx` draft 预算：
  ...
  树生长深度按 `max_beam_len` 配置固定上限执行；运行时动态变化的是 beam candidate、
  budget pruning、proactive hit 后的树重排/续接，以及多请求 validation 的 interleave。

`server_only.draft_token_rate_tok_s` 单独控制 server-only 服务器侧 drafter 的解析速度，
默认 504 tok/s。该值固定自 ComputingForGeeks 的 RTX 4090 + Ollama 实测：
`qwen2.5:1.5b` 在五个 prompt 上的生成速率为 496.4、506.5、509.6、516.6、490.4 tok/s，
均值约 503.9 tok/s。
...
- `avg_tpot_ms`、`avg_tbt_ms`：decode-only TPOT/TBT，当前仿真按请求完成时延除以最终提交 token 数统计；
...
1. 每个场景下主方法和基线的 `avg_latency_ms`、`p95_latency_ms`、`avg_tpot_ms`、`avg_tbt_ms`；
2. `full` 相对 `target_only`、`server_only_linear`、`specedge_linear`、`dip_sd`、
   `server_only_tree` 和 `specedge_tree` 的加速比或时延降低；
...
| 消融目标 | 对比方法 | 观察指标 |
|---|---|---|
| 持续乐观起草收益 | `full` vs `wo_async` | 时延、TPOT/TBT、lane utilization、device queue wait |
...
当前 `full` 已按 DSI-style 持续向后起草，不再读取 `W_default` 作为固定 segment
窗口限制；它读取 `speculation.unconfirmed_token_budget` 作为未确认 draft token 总预算。
因此旧的 `sensitivity-w` 不再适合作为 `full` 的核心敏感性实验；保留该脚本只用于
兼容旧运行流程。若需要分析预算限制，应 sweep `unconfirmed_token_budget`；若需要分析
固定 segment 窗口，应新增一个显式 fixed-window 方法，或直接比较 `full` 与 `wo_async`。
...
- `full` 相对 `wo_async` 的时延、TPOT/TBT 和 stale waste；
...
- `avg_latency_ms`、`p95_latency_ms`、`avg_tpot_ms` 和 `avg_tbt_ms` 是否下降；
...
- `speculation.gamma_candidates`：评估 `full`、`specedge_linear`、`server_only_linear` 和异步消融方法中更保守或更激进的 lookahead；SpecEdge/server-only 树形基线不使用该项选择树深度；
- `speculation.unconfirmed_token_budget`：评估 `full` 和异步消融方法中每个请求允许的未确认 draft token 总预算；
...
- `server_only.batch_size`：主实验固定为 1；大于 1 是可选扩展，当前实现禁止配置而不是退化成单请求执行；
- `specedge.tree_draft_strategy` / `server_only.tree_draft_strategy`：legacy 方法的手动切换项；canonical `specedge_linear`/`server_only_linear` 总是线性，`specedge_tree`/`server_only_tree` 总是树形；
...
- `target_only` 语义最直接，但所有 decode 生成集中在边缘，平均时延和 TPOT/TBT 通常较高；
- `server_only_linear` 和 `server_only_tree` 消除了端边往返，但所有 draft/verify 都集中在服务器侧，且主实验按 `server_only.batch_size=1` 同步执行；
- `specedge_linear` 和 `specedge_tree` 通过端侧 drafter 分担 draft 计算，并引入端边通信、server batch validation 和 proactive continuation；
- `dip_sd` / `DiP-SD (Online Adaptation)` 通过 ordered batch pipeline 降低同步空泡，但 epoch barrier 和有序 batch 会影响尾部时延；
- `specedge_tree` 表示 SpecExec-style 树形近似 baseline，预期通过 server batch validation、
...
- `wo_async` 去掉持续乐观起草后，资源重叠不足，TPOT/TBT 和平均时延通常劣于 `full`；
