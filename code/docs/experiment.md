...
| 方法 | 基线含义 | 设备池与行为 |
|---|---|---|
| `target_only` | 边缘自回归基线 | 请求上传边缘后由单个 target 服务资源自回归生成；首 token 可在第一次 decode 后返回 |
...
- `avg_ttft_ms`：首 token 返回时延；对 `target_only`，它在 prompt 上传、边缘排队、
  prefill、第一次 decode 和首 token 下行后记录，不等待全部输出生成完成；
...
- `target_only` 语义最直接，但所有生成集中在边缘，平均时延通常较高；TTFT 只等待
  prefill 和第一次 decode，因此不会等同于完整请求时延；
