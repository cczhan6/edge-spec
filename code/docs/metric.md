...
| `avg_acceptance_rate` | 真实 accepted draft tokens 除以总 draft tokens。 |
...
| SpecEdge 专有项 | proactive segment、proactive hit/waste、pipeline idle bubble 和 draft-depth alignment error。 |
...
| `segment_details_<scenario>_<method>.csv` | 排查 segment 级 scheduled/verify gamma、真实 acceptance、bonus reuse、payload、lane、SpecEdge pipeline 字段和最终状态。 |
...
`SpecEdge` 的对比字段名为 `latency_ratio_vs_specedge` 和 `relative_latency_reduction_vs_specedge`。其 pipeline-aware scheduling 事件会写入 `event_details` 的 `pipeline_schedule`、`global_batch_verify.batch_type` 和 `pipeline_idle_bubble_ms` 字段。

`server_only` 不产生 `global_batch_verify` 事件。request 明细中的请求级上下行字段记录 prompt 上传和最终输出下载；segment 明细中的上下行时延与 payload 为 0。
